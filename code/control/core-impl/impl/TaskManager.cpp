...
void TaskManager::handleTaskResult(TaskResult &message, ParsedTextMessage &parsedTextMessage) {
    ...
    if(current_message_size + 150 > max_message_size){
        ...
        for(int i = 1; i <= parts; i++){
            ...
            if( i == 1){
                result = generate_section(properties.keys.packageType, properties.packages.firstPart)
                        .append(generate_section(properties.keys.targetType, parsedTextMessage.getTargetType()))
                        .append(generate_section(properties.keys.targetId, parsedTextMessage.getTargetId()))
                        .append(generate_section(properties.keys.targetModule, parsedTextMessage.getModule()))
                        .append(generate_section(properties.keys.responseType, parsedTextMessage.getResponseType()))
        ...
        result = generate_section(properties.keys.packageType, properties.packages.singleMessage) +
                 generate_section(properties.keys.targetType, parsedTextMessage.getTargetType()) +
                 generate_section(properties.keys.targetId, parsedTextMessage.getTargetId()) +
                 generate_section(properties.keys.targetModule, parsedTextMessage.getModule()) +
                 generate_section(properties.keys.responseType, parsedTextMessage.getResponseType()) +
                 generate_section(properties.keys.requestId, parsedTextMessage.getRequestId());
